...
for iters in iterates:
    for title in titles:
        ...
        logger = set_logger(os.path.join(save_path, 'CILRS_{0}_{1}.log'.format(method, title)), title, iters)
        ...
        np.save('PA_CILRS_perturb_{0}_iters{1}.npy'.format(title, iters), etas)
