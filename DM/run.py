...
for noise in noises:
    for title in titles:
        ...
        np.save('DM_CILR_perturb_{0}_noise{1}.npy'.format(title, noise), perturb.data.cpu().numpy())
        logger = set_logger(os.path.join(save_path, 'CILR_{0}_{1}.log'.format('DeepManeuver', title)), title, noise)
        logger.info('method = {0}, direction = {1}, video = {2}, eps={3}, noise={4}, iter={5}'.format('DeepManeuver', 'steering=right', title, 2, noise, iter))
